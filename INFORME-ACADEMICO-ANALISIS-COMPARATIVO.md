...
La implementación en Selenium IDE se realizó utilizando la extensión del navegador para grabar y editar las secuencias de prueba. Se generó un archivo .side que contiene los dos flujos principales de prueba utilizando los comandos nativos de Selenium IDE.
...
Selenium IDE demostró ser significativamente más rápido para la creación inicial de pruebas mediante su interfaz de grabación. La capacidad de capturar acciones del usuario en tiempo real y convertirlas automáticamente en comandos de prueba reduce considerablemente el tiempo de desarrollo inicial. Este enfoque de "record and playback" es especialmente ventajoso para equipos que requieren crear pruebas rápidamente sin necesidad de escribir código.

Por otro lado, Playwright requiere una inversión inicial mayor en la escritura de código TypeScript. Sin embargo, este tiempo adicional se compensa con la creación de pruebas más robustas y mantenibles desde el inicio. El mecanismo de auto-espera integrado en Playwright manejó automáticamente todos los problemas de sincronización sin requerir intervención manual, lo que resulta en pruebas más estables a largo plazo.
...
Las pruebas automatizadas en entornos web pueden presentar fallos relacionados con sincronización. Estos se categorizan típicamente en:

1. **Click Interception**: Elementos técnicamente visibles en el DOM pero cubiertos por otros elementos (z-index, overlays, popups).

2. **Timing Issues**: Situaciones donde el script intenta interactuar con elementos que aún no están completamente cargados o habilitados.

3. **Race Conditions**: Condiciones donde el orden de carga de elementos no es determinístico.
...
**Opinión personal**: En mi experiencia práctica con este proyecto, Playwright demostró mayor estabilidad gracias a su sistema de auto-wait. Las pruebas en Selenium IDE requirieron la inclusión de comandos `pause()` y `waitForElementPresent` para manejar elementos dinámicos, mientras que Playwright gestionó estos casos automáticamente. Esta diferencia impacta significativamente la confiabilidad de las pruebas en proyectos de mayor escala.
